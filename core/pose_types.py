...
def _collinear_leg_zero(body_kps: np.ndarray, height: int) -> np.ndarray:
    ...
    for hip_i, knee_i, ankle_i in [(8, 9, 10), (11, 12, 13)]:
        ...
        ax,ay=h[0],h[1]; bx,by=k[0],k[1]; cx,cy=a[0],a[1]
...
def coco133_to_pose_keypoint(
    ...
    for person_idx in range(len(keypoints)):
        kps = keypoints[person_idx]; scores_ = scores[person_idx]
        ...
        for op_idx in range(18):
            ...
            if isinstance(coco_ref, tuple):
                i,j=coco_ref
                x=float((kps[i,0]+kps[j,0])/2); y=float((kps[i,1]+kps[j,1])/2)
                c=float((scores_[i]+scores_[j])/2)
            else:
                x=float(kps[coco_ref,0]); y=float(kps[coco_ref,1]); c=float(scores_[coco_ref])
        ...
        for fi in range(6):
            ...
            x=float(kps[ci,0]); y=float(kps[ci,1]); c=float(scores_[ci])
            ok = c >= score_threshold and _in_bounds(x, y, width, height)
        ...
        for attr, start in [("hand_left_keypoints_2d", LHAND_START),
            ...
            for i in range(NUM_HAND_KPS):
                x=float(kps[start+i,0]); y=float(kps[start+i,1]); c=float(scores_[start+i])
        ...
        for i in range(NUM_FACE_KPS):
            x=float(kps[FACE_START+i,0]); y=float(kps[FACE_START+i,1]); c=float(scores_[FACE_START+i])
            if c < score_threshold or not _in_bounds(x, y, width, height): x,y,c=0.,0.,0.
...
def pose_keypoint_to_arrays(pose_kp, image_idx=0):
    ...
    entry=pose_kp[image_idx]; w=entry.get("canvas_width",512); h=entry.get("canvas_height",512)
    result=[]
    for person in entry.get("people",[]):
        p={}
        def _parse(key,n):
            flat=person.get(key,[]); arr=np.array(flat,dtype=np.float32)
            return arr[:n*3].reshape(n,3) if len(arr)>=n*3 else np.zeros((n,3),dtype=np.float32)
        p["body"]=_parse("pose_keypoints_2d",18); p["face"]=_parse("face_keypoints_2d",70)
        p["hand_left"]=_parse("hand_left_keypoints_2d",21); p["hand_right"]=_parse("hand_right_keypoints_2d",21)
...
def scale_pose_keypoint(pose_kp,src_w,src_h,dst_w,dst_h):
    ...
    sx=dst_w/max(src_w,1); sy=dst_h/max(src_h,1)
    import copy; result=copy.deepcopy(pose_kp)
    for entry in result:
        entry["canvas_width"]=dst_w; entry["canvas_height"]=dst_h
        for person in entry.get("people",[]):
